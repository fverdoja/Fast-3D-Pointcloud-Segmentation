...
__device__ __forceinline__ void MapData2VBOKProcess(BYTE *d_colorFrame, USHORT *d_depthFrame, double *d_colorCoord, float *VMap, float *RGB, float 
*calib, int n, int m) {
	...
	double colorForDepthX = d_colorCoord[idx*2];
	double colorForDepthY = d_colorCoord[idx*2 + 1];
...
__global__ void MapData2VBOKernel(BYTE *d_colorFrame, USHORT *d_depthFrame, double *d_colorCoord, float *VMap, float *RGB, float *calib, int n, int m) {
...
__device__ __forceinline__ void ThreshProcess (cv::gpu::DevMem2D_<unsigned char> Out, cv::gpu::DevMem2D_<unsigned char> In, unsigned char thresh, float *NMap, int n, int m) {
	...
	float norm_n = 	NMap[3*idx] * NMap[3*idx] +	\
			NMap[3*idx+1]*NMap[3*idx+1] + 	\
			NMap[3*idx+2]*NMap[3*idx+2];
