...
void OffLineFrame::LoadFrame(string filename_depth, string filename_color) {
	...
	_depth_dev_test.upload(depth_test); //on charge l'image dans le GPU dans l'attribut _depth_dev_test de notre frame
	depth_test.release();// On libere l'image du CPU

	color_test = cv::imread(filename_color, CV_LOAD_IMAGE_UNCHANGED);
	_color_dev_test.upload(color_test); //on charge l'image dans le GPU
...
//	checkCudaErrors( cudaMemset(_NMap_dev, 0, 3*_n*_m*sizeof(float)) );
	...
	// Release VBO data 
	checkCudaErrors( cudaGraphicsUnmapResources (1, _Resources_t) );// On rend l'utilisation du GPU à la carte graphique //erreur ici
...
void SegmFrame::Segment() {
	...
	cv::gpu::GpuMat img;
	img.create(getN(), getM(), CV_8UC1);// allocation memoire GPU
	
	cv::gpu::GpuMat img2;
	img2.create(getN(), getM(), CV_8UC1);// allocation memoire GPU
...
//	 Get edges from the red channel
	gpu_cpyN(img, getNMap(), getN(), getM(), 0); // On recupere que les composante Norm de la chnl 0
	//img.download(tmp);// 
	...
	//edgesR.download(tmp);
	//cv::imwrite("Cannych0.png", tmp);


	gpu_cpyN(img, getNMap(), getN(), getM(), 1);
	//img.download(tmp);// 
	//cv::imwrite("normch1.png", tmp);

	cv::gpu::Canny(img, edgesG, 120.0, 30.0, 3);
	//edgesG.download(tmp);
	//cv::imwrite("Cannych1.png", tmp);

	gpu_cpyN(img, getNMap(), getN(), getM(), 2);
	//img.download(tmp);// 
	//cv::imwrite("normch2.png", tmp);

	cv::gpu::Canny(img, edgesB, 120.0, 30.0, 3);
	//edgesB.download(tmp);
	//cv::imwrite("Cannych2.png", tmp);
		
	gpu_Sum(img, edgesR, edgesG, edgesB, getN(), getM());// (moyenne)

	
	img.download(tmp);// ici img = edge of the image
	//cv::imshow("edges", tmp);
	//cv::imwrite("grad_im.png", tmp);
	
	//cvWaitKey(10);  

	//cv::gpu::Canny(img, img, 120.0, 30.0, 3);
	...
	gpu_Thresh(img2, img, 20, getNMap(), getN(), getM());// Segmentation/Binarisation
		
	img2.download(tmp);// img2 = parties segmentees (inverse binarisé de img)
	//cv::imshow("edges", tmp);
...
void SegmFrame::ComputePCA() { 
	...
	int m = getM();
		...
		for (int i = 0; i < _blobsF.size(); i++) {
					...
					cv::Mat Segm_vertex((int)_blobsF[i].size(), 3, CV_32FC1);
					...
					eigenvectors[3] = pos;// on met la position de valeur moyenne comme 4 eme coordonnee des vecteurs propres
...
//	int i = 8;//8 sphere // 2 chair //7 wall
